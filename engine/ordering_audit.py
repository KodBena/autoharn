...
def ordering_exit_addendum(r: dict) -> int | None:
    ...
    not move the exit code at all; 7 iff at least one family is VIOLATED, or a family verdict is
    missing (a defect too -- never silently 0). (Authored as 6 in a worktree branch built in
    parallel with --review-gap, which shipped first and owns 6; the integrator bumped this to 7
    at the merge seam, 2026-07-12 -- one exit code, one owner.)"""
    if r["missing_families"]:
        return 7
    return 7 if r["any_violated"] else None
