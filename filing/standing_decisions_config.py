...
#   last-change: 2026-07-16T02:33:46Z
...
SHAPE: `{"grades": [<word>, ...], "byte_cap": <positive int>}`, both keys optional -- an absent or
malformed entry degrades to the documented default (`grades=["durable"]`, `byte_cap=4000`) with a
stderr WARNING (never a silently widened/narrowed grade set), mirroring
`hooks/stop_clean_exit.py`'s own `_resolve_mode()` validation posture applied to this shape
instead. The kernel (`kernel/lineage/s36-decision-grade.sql`) enforces no vocabulary on
...
def resolve_standing_decisions_config(entry: dict | None) -> tuple[list[str], int]:
    """`entry` = `apparatus["mechanisms"]["standing_decisions"]` (the caller's own extraction --
    see module docstring NAMED CHOICE) -> (grades, byte_cap), defaulted/validated. Never raises --
    a malformed entry degrades to the default, loudly, on stderr."""
    if not isinstance(entry, dict):
        return list(DEFAULT_GRADES), DEFAULT_BYTE_CAP
