...
#   last-change: 2026-07-11T22:21:48Z
...
Three regexes, one per shape, unioned across every `hooks/*.py` file, is `known_mechanisms()`'s
whole implementation. A hook that names its mechanism a fourth way (unlikely, but not
impossible) would go undetected here -- an honest limit, named rather than assumed away: if a
future hook's key never shows up in this registry despite genuinely being read, that is itself a
class-1 recurrence (ADR-0011 Rule 2) asking for a fourth pattern, not evidence the approach is
unsound.
...
def known_mechanisms(hooks_dir: Path | None = None) -> frozenset[str]:
    """Every mechanism name some hook in `hooks_dir` (default: this repo's own `hooks/`) actually
    reads from `apparatus["mechanisms"]`, derived by static extraction (see module docstring) --
    never a hand-typed list. Returns an empty frozenset (never raises) if `hooks_dir` does not
    exist or holds no `.py` files -- a caller with a genuinely empty registry gets an empty set to
    reason about, not a crash; `unknown_mechanism_keys` below treats "nothing known" as "every key
    is unknown", which is the fail-loud posture this module exists to serve, not a silent no-op."""
    d = hooks_dir if hooks_dir is not None else HOOKS_DIR
    found: set[str] = set()
    if not d.is_dir():
        return frozenset()
