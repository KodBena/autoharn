...
#   last-change: 2026-07-11T22:22:35Z
...
`filing/apparatus_registry.py`'s `known_mechanisms()` — a set DERIVED from `hooks/*.py`'s own
source, never a hand-typed second list (see that module's docstring for why: a hand list had
ALREADY drifted once, silently, before this gate existed — `bash_completion` was a real,
wired-in mechanism absent from both the shipped template and its own documentation). A key not in
that derived set is reported, with `filing.apparatus_registry.teach_text`'s exact wording naming
the bad key(s) and the full valid set.
...
they choose, per the library framing CONFIGURATION.md states), so every target is named on the
