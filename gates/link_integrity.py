...
(never silent, per the commission):
  ...
  2. design/ARCHITECTURE.md — carries its own `⚠ STALE (2026-06-27; migrated 2026-07-07, not
     rewritten)` banner: "Its internal path citations ... point at the OLD `claude_harness`
     layout, not autoharn's tree ... carried as a design witness, not authority; a full rewrite
     ... is owed and filed in BACKLOG.md." Patching its paths piecemeal without the rewrite the
     banner itself calls for would manufacture false current-ness — worse than leaving the
     banner's own honest disclaimer standing. Excluded until that filed rewrite lands.
...
EXCLUDE_FILES = {"design/ARCHITECTURE.md"}
...
def main() -> int:
    ...
    print(f"    design/ARCHITECTURE.md — self-declared STALE; rewrite filed in BACKLOG.md")
