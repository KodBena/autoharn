...
#   last-change: 2026-07-18T09:11:33Z
...
usage() {
    ...
    echo "          also applies high_watermark_1.sql + s20 through s43 + s45 + s44 + s46, seeds the stamp secret, and" >&2
...
if [ -n "$NEW_WORLD" ]; then
    echo "-- new-world '$NEW_WORLD': applying high_watermark_1.sql + s20 + s21 + s22 + s23 + s24 + s25 + s26 + s27 + s28 + s29 + s30 + s31 + s32 + s33 + s34 + s35 + s36 + s37 + s38 + s39 + s40 + s41 + s42 + s43 + s45 + s44 + s46 to $DB (schema=$SCHEMA kern=$KERN role=$ROLE) --"
    ...
    echo "   kernel applied (schema $SCHEMA + kernel schema $KERN + role $ROLE, s20 + s21 + s22 + s23 + s24 + s25 + s26 + s27 + s28 + s29 + s30 + s31 + s32 + s33 + s34 + s35 + s36 + s37 + s38 + s39 + s40 + s41 + s42 + s43 + s45 + s44 + s46 included -- s29's migration_epoch naturally seeds 0 on this empty ledger, see that file's own AMENDMENT header; s30 needs no epoch machinery of its own, HISTORY: safe; s40's own birth acts run below, after the seeds; s45 licenses principal_binding_active on the two standing-lifecycle kinds and is honored by the standing declarations below, which now carry the flag; s44 ships the typed model-identity-attestation kind, kernel shape only, no verb writes it yet; s46 ships the view-only credited read surface, model_defeated_rows + credited_current, zero columns/kinds)"
