...
#   last-change: 2026-07-15T20:35:47Z
...
usage() {
    ...
    echo "          also applies high_watermark_1.sql + s20 through s32 and seeds the stamp secret -- see" >&2
...
if [ -n "$NEW_WORLD" ]; then
    LINEAGE_CHAIN="s15 -> s17-stamp-mechanism -> s17-independence-vocabulary -> s19 -> s20 -> s21-session-aware-distinctness -> s22-work-item-ledger -> s23-per-invocation-stamp-token -> s24-declared-event-time -> s25-commission-kind -> s26-row-hash-chain -> s27-chain-high-water -> s28-work-parent-edge -> s29-obligation-item-key-and-typed-close -> s30-typed-dependency-edges -> s31-supersession-uniform-retraction -> s32-edge-views-single-home (via kernel/lineage/high_watermark_1.sql + kernel/lineage/s20-obligation-grants-and-view-refresh.sql + kernel/lineage/s21-session-aware-distinctness.sql + kernel/lineage/s22-work-item-ledger.sql + kernel/lineage/s23-per-invocation-stamp-token.sql + kernel/lineage/s24-declared-event-time.sql + kernel/lineage/s25-commission-kind.sql + kernel/lineage/s26-row-hash-chain.sql + kernel/lineage/s27-chain-high-water.sql + kernel/lineage/s28-work-parent-edge.sql + kernel/lineage/s29-obligation-item-key-and-typed-close.sql + kernel/lineage/s30-typed-dependency-edges.sql + kernel/lineage/s31-supersession-uniform-retraction.sql + kernel/lineage/s32-edge-views-single-home.sql), applied automatically by this --new-world run -- s29 wired in via its sec-10 migration-epoch amendment (ledger decision row 935's conditional ratification), which yields epoch=0 on this empty, freshly-scaffolded ledger (see that file's own AMENDMENT header for why); s30 (typed dependency edges, ledger decision row 1018) needs no epoch machinery of its own -- HISTORY: safe, see that file's own header; s31 (supersession uniform retraction, ratified spec design/FABLE-SUPERSESSION-UNIFORM-RETRACTION-SPEC.md) re-issues readers only, no epoch, HISTORY: safe per its own header; s32 (edge-views-single-home) is a pure refactor, output-equality witnessed, HISTORY: safe per its own header"
...
if [ -n "$NEW_WORLD" ]; then
    echo "-- new-world '$NEW_WORLD': applying high_watermark_1.sql + s20 + s21 + s22 + s23 + s24 + s25 + s26 + s27 + s28 + s29 + s30 + s31 + s32 to $DB (schema=$SCHEMA kern=$KERN role=$ROLE) --"
    ...
    echo "   kernel applied (schema $SCHEMA + kernel schema $KERN + role $ROLE, s20 + s21 + s22 + s23 + s24 + s25 + s26 + s27 + s28 + s29 + s30 + s31 + s32 included -- s29's migration_epoch naturally seeds 0 on this empty ledger, see that file's own AMENDMENT header; s30 needs no epoch machinery of its own, HISTORY: safe)"
