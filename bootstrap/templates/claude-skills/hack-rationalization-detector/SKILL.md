...
**The return path is part of the frame.** Independence on the way IN is worth
nothing if a non-independent layer paraphrases the result on the way OUT. The
documented failure of THIS skill was exactly that: an out-of-frame auditor
correctly found "nothing architecturally prevents bug type A from recurring",
and the orchestrating agent collapsed it to a one-word "verdict: GENERAL" and
dropped the finding before it reached the maintainer. That is the same
named-and-bypassed disease, relocated to the synthesis layer. So:

- The audit's full output (the artifact below, verbatim) is the deliverable.
  Write it to a file and surface THAT to the maintainer directly. The raw
  artifact reaches them unmediated.
- An orchestrating agent may **append** ("I agree", "added context", "I'd
  prioritize finding 2") but may **never replace, summarize, soften, or omit**
  any line of the auditor's output. The verdict label is not permitted to
  travel alone. If you find yourself compressing the audit into a cleaner
  headline, that compression is the bug — stop and pass the artifact through.
...
FINDINGS BEYOND VERDICT:
  - <anything TRUE you noticed that the verdict does not capture — architectural
    gaps, "nothing prevents X from recurring", residual fragility, a frame the
    evidence doesn't support. REQUIRED even when VERDICT is general.>
```

A verdict compresses; the maintainer value lives in what gets compressed away.
A single-token verdict is lossy by design, so the FINDINGS section carries the
substance the label cannot. This section is **mandatory and may not be empty on
a non-trivial audit** — a clean "general" verdict with no findings on a large
review is itself suspicious, because real audits of real systems almost always
leave residue. If you genuinely found nothing beyond the verdict, say so
explicitly ("no residual findings") rather than omitting the section; an empty
section and an absent section are not the same signal. The verdict can be
correct on the change under review *and* the findings can hold the thing that
actually matters to the maintainer — surface both.
...
`references/known-cases.md` is **GENERATED** (do not hand-edit it) — its cases are
the seed plus every fire that has been filed and confirmed in the rationalization
ledger. Filing a fire is how a new case becomes a prior case; see the filing
...
FINDINGS BEYOND VERDICT (required; "no residual findings" if truly none):
  - <true things the verdict doesn't capture — architectural gaps, "nothing
    prevents X from recurring", residual fragility, an unsupported frame>
```

This entire block is the deliverable. Write it to a file and surface it to the
maintainer verbatim. If you are an orchestrator relaying an auditor's run, you
may append your own notes below it but you may not edit, summarize, or drop any
line above — the verdict label must never travel without its body and findings.

## Filing a fire — making a case a prior case (the append path)

The detector's outcomes are a corpus, not incidents. Every fire (and its eventual
adjudication) is filed into the **rationalization ledger** — a labeled SQL store in
the harness DB (`harness` schema) — so the corpus is queryable (which ADR
formulations get rationalized around most; the false-positive rate) and so a
confirmed fire actually becomes a prior case in the few-shot above. **You file
through `tools/file_rationalization.py`, never a hand-written INSERT.** (Repo:
`claude_harness`; store DDL: `db/harness/002_rationalization_ledger.sql`.)

1. **File the fire** (the moment the audit produces a GENERAL/PATCH/DOWNGRADE
   artifact worth keeping — a real fire, not a clean pass):

   ```bash
   python tools/file_rationalization.py file \
     --quoted "<the verbatim rationalization the tell scanner caught>" \
     --register "<the discipline-word used, e.g. 'scope creep' / 'for now'>" \
     --better-fix "<the named-and-downgraded more-general fix, if one was named>" \
     --law-refs "<ADR refs routed around, e.g. 'ADR-0013 R3; ADR-0000 R2(a)'>" \
     --context "<the change/diff/PR/file the fire was on>" \
     --detector-version "<this skill's version>" [--session <id>] [--commit <sha>]
   ```
   It prints the `finding_id` (idempotent: re-filing the same fire returns the same id).

2. **Dispose it** once adjudicated — an actor-attributed, append-only act (F28:
   nothing auto-resolves; to reverse a judgment you append a NEW act, never edit):

   ```bash
   python tools/file_rationalization.py dispose --finding <id> \
     --act confirmed-hack|false-positive|duplicate-of --actor "<who>" \
     [--duplicate-of <other_finding_id>] [--note "<why>"]
   ```

3. **Regenerate the few-shot** so a confirmed case joins `references/known-cases.md`:

   ```bash
   python tools/file_rationalization.py gen-known-cases
   ```
   Only `confirmed-hack` findings become known cases; the curated seed always leads.
...
- This skill cannot defend its own output against the layer that relays it. The
  verbatim-return rule is a policy, not an enforced gate — an orchestrator that
  ignores it will still launder the result. The only real defense is the
  maintainer reading the raw artifact directly rather than a relayed summary.
  If you are the maintainer: read the auditor's file, not anyone's gloss of it.
