...
```json
{
  ...
  "mechanisms": {
    ...
    "error_recurrence": {"mode": "observe", "note": "..."}
  }
}
```
...
## The fourteen mechanisms and their defaults

The table below lists every mechanism this project ships, the file that implements it (a hook,
for thirteen of the fourteen; a `.tmpl` operator verb for one, `doc_attestation` — its
...
| `doc_attestation`      | `bootstrap/templates/distance-to-clean.tmpl` (NOT a hook — see this mechanism's own dedicated note, below the table, for the full explanation) | **`off`** | **free per call** (pure hashing, no LLM call, no network) — OFF anyway, because this switch is not about cost: it gates whether `./distance-to-clean` (the operator verb that reads a deployment's outstanding closure debt — unreviewed decisions, open questions, work-item violations — in one pass; `bootstrap/templates/distance-to-clean.tmpl`) counts a DOC-ATTESTATION section's debt (the block `./distance-to-clean` prints listing which documents lack a fresh-context legibility sign-off) for the ADR-0017 A:B:C fresh-context audit loop (a three-role review workflow — one agent writes a document, a second reviews it having seen only the document, a third repairs whatever the second found; spelled out in full further down this document, in the `doc_legibility_critic` row's own note), a workflow a deployment adopts by choice (`design/ORCH-SPEC-ABC-OFFERING.md` §4). `enforce` is **not applicable** (this section only ever reports, it has no deny path) — degrades to `observe` with a warning, same shape as `mutation_observer`/`bash_completion`. Added 2026-07-12 (tracker item `abc-loop-offering` — a short slug this project's own commit history and design docs use to cross-reference the work that introduced a change; not a separate tracked system with its own lookup page, so `git log --all --grep=abc-loop-offering` from the repo root is how a reader finds the commit(s) it names) |
| `error_recurrence`     | `hooks/posttooluse_error_recurrence.py`         | `observe` | **spends nothing live-per-call beyond this world's own ledger read** (no LLM call, no network beyond the ledger) — same costless-observer convention as `bash_completion`/`mutation_observer`/`read_observer`. On a PostToolUse(Bash) result matching one of a small, enumerated set of ERROR SIGNATURES (a SQLSTATE refusal line, a house `REFUSED` line, or a gate `VIOLATIONS (n)` line — see the hook's own module docstring for the exact three, and the NAMED RESIDUAL GAP it discloses: a Bash call's own non-zero exit status does not itself deliver a `PostToolUse` payload to inspect, witnessed empirically against Claude Code 2.1.214), it read-only cross-checks this world's ledger for a matching `defect:` prior (user-guide/USER-RECIPES-FAQ.md "Capturing errors so they cannot quietly recur") and surfaces it via `additionalContext` — **detective control only** (`design/FABLE-ERROR-RECURRENCE-HOOK-SPEC.md`, ledger row 1697): never blocks, never writes a ledger row, never polices. `enforce` is **NAMED-IMPOSSIBLE**, same shape as `mutation_observer`/`bash_completion` but for a stronger reason than "PostToolUse cannot deny" alone — the spec itself binds this mechanism to never block, full stop; the hook warns loudly and behaves as `observe` if apparatus.json ever names `enforce` here. **Ships `"observe"` in THIS shipped template's config — but "ships the switch set to observe" is NOT the same as "starts firing"**: this repo's OWN live `.claude/apparatus.json` deliberately carries no entry at all for this key (the hook's internal default, absent any entry, is `"off"`, not `"observe"` — the one deliberate departure from the costless-observer-defaults-on convention every sibling in this table follows, because THE ONE HARD SAFETY RULE this build was shaped by requires the mechanism to be inert everywhere until an operator deliberately arms it; see the hook's own module docstring), AND, unlike every hook above, it is not yet wired into `bootstrap/templates/settings.json.tmpl`'s own `PostToolUse` dispatch at all — so even a fresh scaffold that copies this `"observe"` default verbatim never actually invokes the hook until an operator adds the dispatch line by hand (see `HOOKS.md.tmpl`'s own note). A fresh scaffold therefore gets the config default and this doc, not a live-firing hook. Added 2026-07-19 (`design/FABLE-ERROR-RECURRENCE-HOOK-SPEC.md`, ledger row 1697, superseding row 1696's hold) |
...
- **`demurral_detect`** also carries per-mechanism SETTINGS next to its mode: `classifier_command`
  ...
  strength, not a variant of an existing one:
    ...
    - **The phrase list AND the injected notice text are both DATA, not code constants**
      ([ADR-0012](../../law/adr/0012-compositional-and-structural-hygiene.md)'s data/code-separation discipline, applied here 2026-07-17 on the maintainer's
...
which mechanisms' `mode` values changed, when the content parses. It closes a real gap the thirteen
hook-implemented mechanisms above left open (thirteen, not fourteen: `doc_attestation` is read by a
`.tmpl` verb, not by any hook, so it never watches anything): none of them unconditionally watches an edit to THIS file —
