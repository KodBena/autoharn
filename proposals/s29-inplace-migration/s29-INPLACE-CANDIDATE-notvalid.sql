...
ALTER TABLE :"schema".ledger ADD CONSTRAINT work_review_disposition_kind_shape CHECK (
    (kind = 'work_closed') = (work_review_disposition IS NOT NULL)) NOT VALID;
-- ^^^ IN-PLACE-MIGRATION CANDIDATE (UNRATIFIED), 2026-07-14: sole difference from the ratified
-- s29 file (worktree-agent-a4f06b5d7e62f03c2 @ 92e1c75) is this constraint's NOT VALID marker.
-- WHY: ent carries 157 work_closed rows that legitimately predate the disposition type; the
-- two-way CHECK is enforced on every FUTURE insert (witnessed below in the rehearsal), while
-- the 157 historical rows are honestly exempt rather than backfilled with fabricated facts.
-- This file lives in a scratchpad, is NOT kernel/lineage, and applies to nothing without a
-- Fable-authored, maintainer-ratified spec adopting it.
