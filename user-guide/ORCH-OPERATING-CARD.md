...
> (`extract-context`, `orchlog`, `otel-attest`, `otel-watch` were never part of this shim
> family and are untouched. `courier` was also never part of this deprecation-alias shim
> family, but a DATED amendment, not a silent rewrite: work item courier-umbrella-rebase,
> ledger rows 1369/1370, 2026-07-26 — courier has since been rebased into
> `libexec/autoharn/courier` and wired into `./autoharn`'s own dispatch table, the same way
> led/pickup/judge were; reach it via `autoharn courier`, never a bare `./courier`.)
