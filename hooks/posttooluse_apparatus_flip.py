...
-- journals ONE line to `<SUBJECT_ROOT>/.claude/logs/apparatus_flip.journal.jsonl`:
  {"ts": ..., "event": "apparatus_flip",
   "baseline_kind": "prior_observation" | "first_observation_vs_scaffold_default",
   "tool": <best-effort tool name, or null for the day-zero kind -- see BASELINE below>,
...
BASELINE: the FIRST invocation in a world's life (no persisted state file yet) does NOT blindly
trust whatever apparatus.json currently contains (an out-of-frame hack-rationalization audit,
2026-07-12, caught an earlier version of this file doing exactly that -- CLAUDE.md's engineering-
responsibility corollary in action, not a corner routed around: "session start is not itself a
flip" quietly meant "the world's very FIRST-ever mutation of apparatus.json, if it happens before
this hook has ever run once, is silently absorbed as trusted" -- the single highest-value blind
spot for the threat this hook exists to close). Instead, the first observation is compared against
`bootstrap/templates/apparatus.json` -- this project's own shipped scaffold default, reachable via
`_REPO_ROOT` the same way every sibling hook already resolves its own repo-relative paths -- and a
mismatch journals a `first_observation_vs_scaffold_default` flip (before = the scaffold default's
state, after = current state; `tool` is honestly `null`, since which tool call produced the
divergence was never observed, only that one occurred before monitoring began). This closes the
day-zero gap for the common case (a `--new-world` scaffold, where the shipped default IS the
world's true birth state). It does NOT close it for a world where the scaffold default is
unreachable (this hook copied/invoked outside an autoharn checkout) or where apparatus.json's very
first observed state already matches the scaffold's default byte-for-byte while ALSO being the
result of an already-executed flip that happened to restore that exact content (a vanishingly
narrow case, named for completeness, not hidden) -- in EITHER of those, first observation still
establishes the baseline silently, an honestly disclosed residual: no monitor can detect a change
that happened before it existed and left no comparable reference. Every later call compares against
the LAST flip-or-baseline recorded, so a run of N identical calls between two real content changes
journals exactly one event per actual change, never once per call.
...
STATE_PATH = ""
JOURNAL_PATH = ""
# The shipped scaffold default (bootstrap/templates/apparatus.json) -- repo-relative to THIS
# hook's own file, resolved once here rather than per-call (it never depends on hook-input data,
# unlike SUBJECT_ROOT). Used only to close the day-zero baseline gap (see module docstring).
SCAFFOLD_DEFAULT_PATH = os.path.join(_REPO_ROOT, "bootstrap", "templates", "apparatus.json")
...
def _read_state_of(path: str) -> dict:
    """{"present": bool, "hash": sha256-hex|None, "malformed": bool, "mechanisms": dict|None} for
    the file at `path`. Never raises: every failure mode (missing file, unreadable bytes, invalid
    JSON, a JSON value that is not an object) degrades to an honest state rather than an exception
    -- the fail-safe deliverable in code. Shared by `_read_current_state()` (APPARATUS_PATH) and
    the day-zero scaffold-default comparison (SCAFFOLD_DEFAULT_PATH) -- ADR-0012 P1, one home for
    "what does this apparatus.json-shaped file's state mean," never two re-derivations of it."""
...
def _read_current_state() -> dict:
    return _read_state_of(APPARATUS_PATH)
...
def _emit_warning(before: dict, after: dict, mechanisms_changed: dict, tool: str | None,
    ...
    tool_clause = (f"during this session via a {tool} call" if kind == "prior_observation" else
                   "detected at this world's FIRST-ever apparatus-flip observation, against the "
                   "shipped scaffold default -- the tool call that produced it was never itself "
                   "observed (day-zero comparison, see module docstring)")
    warning = (
        "[apparatus-flip WARNING, observer-mode -- never blocks] .claude/apparatus.json changed "
        f"{tool_clause} (hooks/posttooluse_apparatus_flip.py):\n"
...
def _report_flip(before: dict, current: dict, tool: str | None, kind: str, now: str) -> None:
    """One home for "a flip was detected, journal and warn about it" (ADR-0012 P1) -- shared by
    the ordinary prior-observation compare in `main()` and the day-zero scaffold-default compare,
    so the two paths cannot silently drift into two different record shapes."""
    ...
    _journal({"ts": now, "event": "apparatus_flip", "baseline_kind": kind, "tool": tool,
              "before": before_out, "after": after_out,
              "mechanisms_changed": mechanisms_changed})
    _emit_warning(before_out, after_out, mechanisms_changed, tool, kind)
...
def main() -> int:
    ...
    try:
        ...
        if before is None:
            # First-ever observation for this world (day-zero, module docstring's BASELINE
            # section): compare against the shipped scaffold default rather than trusting
            # whatever apparatus.json currently holds. A reachable default that DIFFERS from the
            # current state means a flip happened before this hook ever ran -- report it, with an
            # honestly-null `tool` (which call produced it was never observed). A missing default,
            # or one that matches, falls through to the disclosed residual: establish the baseline
            # silently, no event -- there is genuinely nothing left to compare against.
            # hash=None iff not present (see `_read_state_of`), so a hash compare alone already
            # subsumes a presence compare -- no second, redundant condition needed (ADR-0012 P1).
            default_state = _read_state_of(SCAFFOLD_DEFAULT_PATH)
            if default_state.get("present") and default_state.get("hash") != current.get("hash"):
                _report_flip(default_state, current, None, "first_observation_vs_scaffold_default", now)
        ...
        _report_flip(before, current, tool, "prior_observation", now)
        _save_baseline(current, now)
