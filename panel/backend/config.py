...
#   last-change: 2026-07-15T01:04:44Z
...
`filing/pghost_resolve.py` -- reproducing their resolution order exactly, never a hardcoded
host (BUILD SPEC v2 r5 sec 1/11). `filing/pghost_resolve.py` is the ONE home for host
resolution -- NOT `instruments/pghost_resolve.py` (a near byte-for-byte duplicate flagged as a
hazard in sec 13 of the spec and out of this package's scope to de-duplicate; WP-4 files the
tracker item `pghost-resolve-duplicate-home`). Both modules this file needs live outside this
package's tree (repo-root `filing/`), so this module inserts ONE directory onto `sys.path` --
at import time, top-of-file, exactly as `filing/pghost_resolve.py` itself does for
`filing/deployment_record.py` (both live in `filing/`, so THAT module needs no sys.path hop at
all) -- and imports them as ordinary top-level names (no lazy import; gates/no_lazy_imports.py
applies to this file like every other).
"""
...
# API layer can 400 on an unrecognized value BEFORE shelling out, per spec sec 4's contract, AND
# so `GET /api/health` can serve them live to the frontend (spec sec 4, closing round-1 finding
# 1 -- a cross-process fact now has exactly one authoritative definition, read live, never
# hand-copied into app.js). This does not pre-block a value the kernel itself would refuse (e.g.
# `managerial` in an unstamped deployment) -- it only blocks a value that is not even in the
# kernel's vocabulary at all.
...
# Status vocabulary a decomposition item's live disposition renders as (spec sec 3/4/5). Named
# once, read by disposition.py (the pure derivation) and app.py (the API's `status` field)
# alike. AMBIGUOUS (spec sec 3/4, closing round-1 finding 2) is a property of the GROUP, never
# produced by derive_status itself -- see disposition.py.
STATUS_VALUES: tuple[str, ...] = ("OPEN", "WITNESSED", "PARTIAL", "COSIGNED", "AMBIGUOUS")

# Default principal name the panel co-signs AS (spec sec 6). `commissioner` (already registered,
...
class PanelConfig:
    ...
    NOTE: no `manifests_dir` field (spec r5's whole point -- the decomposition lives in the
    ledger, never in a git-resident manifest; `panel/manifests/` is condemned and removed by
    this same package, sec 8's CONDEMNED-AND-REMOVED list)."""
    repo_root: Path
...
def load_config(repo_root: Path | None = None) -> PanelConfig:
    ...
    `filing/pghost_resolve.py.resolve_pghost` -- env vars first (this panel's own
