...
def _retired_track_work_arc() -> int:
    ...
    `./asof-export` itself is still live, current behavior on any normal deployment -- this
    fixture's coverage should someday be rebuilt on `bootstrap/new-project.sh --profile tracker`
    (or `--new-world`), not silently dropped; no rebuild attempted here."""
    print("seen-red/asof-export/run_fixtures.py: RETIRED-SCAFFOLD 2026-07-25 (ledger row 1271)")
    print("  -- this fixture's ADOPT step used bootstrap/track-work.sh, now a teaching-refusal")
    print("  stub. ./asof-export itself is still live; rebuild this fixture's coverage on")
    print("  --profile tracker someday (not attempted here).")
    return 0
