...
def _retired_track_work_arc() -> int:
    ...
    The work-depends default-type advisory behavior itself is still live, current behavior on
    any normal deployment -- this fixture's coverage should someday be rebuilt on
    `bootstrap/new-project.sh --profile tracker` (or `--new-world`), not silently dropped; no
    rebuild attempted here."""
    print("seen-red/led-work-depends-default-type-advisory/run_fixtures.py: RETIRED-SCAFFOLD")
    print("  2026-07-25 (ledger row 1271) -- this fixture's ADOPT step used")
    print("  bootstrap/track-work.sh, now a teaching-refusal stub. The advisory behavior itself")
    print("  is still live; rebuild this fixture's coverage on --profile tracker someday (not")
