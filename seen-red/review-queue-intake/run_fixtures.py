...
def _retired_track_work_arc() -> int:
    ...
    The `review:`/`review-done:` intake-validation feature itself is still live, current
    behavior on any normal deployment -- this fixture's coverage should someday be rebuilt on
    `bootstrap/new-project.sh --profile tracker` (or `--new-world`), not silently dropped; no
    rebuild attempted here."""
    print("seen-red/review-queue-intake/run_fixtures.py: RETIRED-SCAFFOLD 2026-07-25 (ledger")
    print("  row 1271) -- this fixture's ADOPT step used bootstrap/track-work.sh, now a")
    print("  teaching-refusal stub. The review-queue feature itself is still live; rebuild this")
    print("  fixture's coverage on --profile tracker someday (not attempted here).")
    return 0
