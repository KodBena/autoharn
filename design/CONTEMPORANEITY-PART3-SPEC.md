...
[design/CONTEMPORANEITY-AUDIT.md](CONTEMPORANEITY-AUDIT.md) already capture, under exactly
...
| E9 | `stop_disposition_window_ms(N)` | new threshold fact in the thresholds file, with the same measured-derivation-comment obligation the existing three facts carry (measure from [design/RETROSPECTIVE-RUN10.md](RETROSPECTIVE-RUN10.md) / [design/RETROSPECTIVE-RUN11.md](RETROSPECTIVE-RUN11.md) and from the ledgers+journals of the first worlds born with s23 in their birth chain, i.e. runs 9 onward; a provisional value must be derived from those measurements in the fact's own comment, never guessed round) |
