...
<!-- doc-attest-exempt: v1.1 release-cut mechanical edit (de-linked dangling references into the removed MAINT-PG-HBA-HARDENING.md, no prose rewrite). Removal condition: strike this marker and run the real A:B:C loop next time this file is touched for content, not just link repair. -->
...
- `SELECT rolname, rolpassword IS NOT NULL, rolsuper FROM pg_authid WHERE rolname='bork'`
  ...
  the maintainer-internal pg_hba hardening write-up (not part of this release) was written to close and whose status
...
(the maintainer-internal pg_hba hardening write-up, not part of this release) has sat unapplied; CAPABILITIES.md's "Not yet
...
2. **Apply the prepared pg_hba hardening** (maintainer-internal write-up, not part of this
   release: password + scram host lines). S; maintainer act; closes Gap 2's door.
