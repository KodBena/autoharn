...
- `SELECT rolname, rolpassword IS NOT NULL, rolsuper FROM pg_authid WHERE rolname='bork'`
  ...
  `design/PG-HBA-HARDENING.md` was written to close and whose status
...
(`PG-HBA-HARDENING.md`) has sat unapplied; CAPABILITIES.md's "Not yet
...
2. **Apply the prepared pg_hba hardening** (`PG-HBA-HARDENING.md`:
...
- [design/GPG-TRUST-LAYER.md](MAINT-GPG-TRUST-LAYER.md) and
  ...
  Tier 1 arms.
- `design/PG-HBA-HARDENING.md` — the prepared perimeter fix.
