...
if [ "$PHASE_CLAIM_ITEMS_FRESH" -eq 1 ]; then
  out=$("$LED" work depends "$SLUG_CLAIM_ITEMS" "$SLUG_OPEN_ITEMS" --type blocks-start 2>&1); rc=$?
  echo "$out"
...
if [ "$PHASE_CLOSE_JUDGMENT_FRESH" -eq 1 ]; then
  out=$("$LED" work depends "$SLUG_CLOSE_JUDGMENT" "$SLUG_CLAIM_ITEMS" --type blocks-start 2>&1); rc=$?
  echo "$out"
...
if [ "$PHASE_CLOSE_COMPANION_FRESH" -eq 1 ]; then
  out=$("$LED" work depends "$SLUG_CLOSE_COMPANION" "$SLUG_CLOSE_JUDGMENT" --type blocks-start 2>&1); rc=$?
  echo "$out"
...
if [ "$NO_OBLIGATE" -eq 1 ]; then
  ...
  if [ "$CONFIRMED" -eq 1 ]; then
    out=$("$LED" obligate "faq-bookkeeping-close-pairing-${INSTANCE}-review" "$OBLIGATE_ASSIGNED_BY" "$OBLIGATE_OBLIGED_ACTOR" 2>&1)
    rc=$?
    if [ $rc -eq 0 ]; then
      echo "$out"
