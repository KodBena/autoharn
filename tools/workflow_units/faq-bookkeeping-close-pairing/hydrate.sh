...
# `faq-bookkeeping-close-pairing-<instance>-<phase>`, `--instance <token>` is MANDATORY, and idempotent-by-refusal
...
usage() {
  ...
  echo "                      slugs are faq-bookkeeping-close-pairing-<instance>-<phase>. Two different tokens are two" >&2
...
echo "-- workflow-unit hydration: faq-bookkeeping-close-pairing instance '$INSTANCE' (source design/workflows/faq-bookkeeping-close-pairing.toml) --"
echo "-- led: $LED --"
...
SLUG_OPEN_ITEMS="faq-bookkeeping-close-pairing-${INSTANCE}-open-items"
SLUG_CLAIM_ITEMS="faq-bookkeeping-close-pairing-${INSTANCE}-claim-items"
SLUG_CLOSE_JUDGMENT="faq-bookkeeping-close-pairing-${INSTANCE}-close-judgment"
SLUG_CLOSE_COMPANION="faq-bookkeeping-close-pairing-${INSTANCE}-close-companion"

open_phase "$SLUG_OPEN_ITEMS" "open-items phase of faq-bookkeeping-close-pairing instance '${INSTANCE}' (from design/workflows/faq-bookkeeping-close-pairing.toml)"
PHASE_OPEN_ITEMS_FRESH=$?

open_phase "$SLUG_CLAIM_ITEMS" "claim-items phase of faq-bookkeeping-close-pairing instance '${INSTANCE}' (from design/workflows/faq-bookkeeping-close-pairing.toml)"
PHASE_CLAIM_ITEMS_FRESH=$?

open_phase "$SLUG_CLOSE_JUDGMENT" "close-judgment phase of faq-bookkeeping-close-pairing instance '${INSTANCE}' (from design/workflows/faq-bookkeeping-close-pairing.toml)"
PHASE_CLOSE_JUDGMENT_FRESH=$?

open_phase "$SLUG_CLOSE_COMPANION" "close-companion phase of faq-bookkeeping-close-pairing instance '${INSTANCE}' (from design/workflows/faq-bookkeeping-close-pairing.toml)"
PHASE_CLOSE_COMPANION_FRESH=$?


if [ "$PHASE_CLAIM_ITEMS_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_CLAIM_ITEMS" "$SLUG_OPEN_ITEMS" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge $SLUG_CLAIM_ITEMS -> $SLUG_OPEN_ITEMS" >&2; exit 1; }
else
  echo "$SLUG_CLAIM_ITEMS: skipping depends_on edge to $SLUG_OPEN_ITEMS (phase already hydrated)"
fi
if [ "$PHASE_CLOSE_JUDGMENT_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_CLOSE_JUDGMENT" "$SLUG_CLAIM_ITEMS" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge $SLUG_CLOSE_JUDGMENT -> $SLUG_CLAIM_ITEMS" >&2; exit 1; }
else
  echo "$SLUG_CLOSE_JUDGMENT: skipping depends_on edge to $SLUG_CLAIM_ITEMS (phase already hydrated)"
fi
if [ "$PHASE_CLOSE_COMPANION_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_CLOSE_COMPANION" "$SLUG_CLOSE_JUDGMENT" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge $SLUG_CLOSE_COMPANION -> $SLUG_CLOSE_JUDGMENT" >&2; exit 1; }
else
  echo "$SLUG_CLOSE_COMPANION: skipping depends_on edge to $SLUG_CLOSE_JUDGMENT (phase already hydrated)"
fi
...
if [ "$NO_OBLIGATE" -eq 1 ]; then
  ...
  if [ "$CONFIRMED" -eq 1 ]; then
    out=$(led_run obligate "faq-bookkeeping-close-pairing-${INSTANCE}-review" "$OBLIGATE_ASSIGNED_BY" "$OBLIGATE_OBLIGED_ACTOR" 2>&1)
    rc=$?
    if [ $rc -eq 0 ]; then
      echo "$out"
...
echo "-- hydration complete for faq-bookkeeping-close-pairing instance '$INSTANCE' --"
