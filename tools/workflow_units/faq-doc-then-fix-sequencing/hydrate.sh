...
echo "-- workflow-unit hydration: faq-doc-then-fix-sequencing (source design/workflows/faq-doc-then-fix-sequencing.toml) --"
echo "-- led: $LED --"
...
open_phase "faq-doc-then-fix-sequencing-doc-authoring" "doc-authoring phase of faq-doc-then-fix-sequencing (from design/workflows/faq-doc-then-fix-sequencing.toml)"
PHASE_DOC_AUTHORING_FRESH=$?

open_phase "faq-doc-then-fix-sequencing-ledger-citation" "ledger-citation phase of faq-doc-then-fix-sequencing (from design/workflows/faq-doc-then-fix-sequencing.toml)"
PHASE_LEDGER_CITATION_FRESH=$?

open_phase "faq-doc-then-fix-sequencing-fix-authoring" "fix-authoring phase of faq-doc-then-fix-sequencing (from design/workflows/faq-doc-then-fix-sequencing.toml)"
PHASE_FIX_AUTHORING_FRESH=$?


if [ "$PHASE_LEDGER_CITATION_FRESH" -eq 1 ]; then
  out=$("$LED" work depends "faq-doc-then-fix-sequencing-ledger-citation" "faq-doc-then-fix-sequencing-doc-authoring" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge faq-doc-then-fix-sequencing-ledger-citation -> faq-doc-then-fix-sequencing-doc-authoring" >&2; exit 1; }
else
  echo "faq-doc-then-fix-sequencing-ledger-citation: skipping depends_on edge to faq-doc-then-fix-sequencing-doc-authoring (phase already hydrated)"
fi
if [ "$PHASE_FIX_AUTHORING_FRESH" -eq 1 ]; then
  out=$("$LED" work depends "faq-doc-then-fix-sequencing-fix-authoring" "faq-doc-then-fix-sequencing-ledger-citation" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge faq-doc-then-fix-sequencing-fix-authoring -> faq-doc-then-fix-sequencing-ledger-citation" >&2; exit 1; }
else
  echo "faq-doc-then-fix-sequencing-fix-authoring: skipping depends_on edge to faq-doc-then-fix-sequencing-ledger-citation (phase already hydrated)"
fi
...
if [ "$NO_OBLIGATE" -eq 1 ]; then
  ...
  if [ "$CONFIRMED" -eq 1 ]; then
    out=$("$LED" obligate "faq-doc-then-fix-sequencing-review" "$OBLIGATE_ASSIGNED_BY" "$OBLIGATE_OBLIGED_ACTOR" 2>&1)
    rc=$?
    if [ $rc -eq 0 ]; then
      echo "$out"
...
echo "-- hydration complete for faq-doc-then-fix-sequencing --"
