...
if [ "$PHASE_LEDGER_CITATION_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_LEDGER_CITATION" "$SLUG_DOC_AUTHORING" --type blocks-start 2>&1); rc=$?
  echo "$out"
...
if [ "$PHASE_FIX_AUTHORING_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_FIX_AUTHORING" "$SLUG_LEDGER_CITATION" --type blocks-start 2>&1); rc=$?
  echo "$out"
...
if [ "$NO_OBLIGATE" -eq 1 ]; then
  ...
  if [ "$CONFIRMED" -eq 1 ]; then
    out=$(led_run obligate "faq-doc-then-fix-sequencing-${INSTANCE}-review" "$OBLIGATE_ASSIGNED_BY" "$OBLIGATE_OBLIGED_ACTOR" 2>&1)
    rc=$?
    if [ $rc -eq 0 ]; then
      echo "$out"
