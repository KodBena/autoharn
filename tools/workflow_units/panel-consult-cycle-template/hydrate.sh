...
if [ "$PHASE_COMPLIANCE_REVIEW_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_COMPLIANCE_REVIEW" "$SLUG_IMPLEMENT" --type blocks-start 2>&1); rc=$?
  echo "$out"
...
if [ "$PHASE_COMPLIANCE_COUNTERSIGN_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_COMPLIANCE_COUNTERSIGN" "$SLUG_COMPLIANCE_REVIEW" --type blocks-start 2>&1); rc=$?
  echo "$out"
...
if [ "$NO_OBLIGATE" -eq 1 ]; then
  ...
  if [ "$CONFIRMED" -eq 1 ]; then
    out=$(led_run obligate "panel-consult-cycle-template-${INSTANCE}-review" "$OBLIGATE_ASSIGNED_BY" "$OBLIGATE_OBLIGED_ACTOR" 2>&1)
    rc=$?
    if [ $rc -eq 0 ]; then
      echo "$out"
