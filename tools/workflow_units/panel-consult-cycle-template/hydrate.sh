...
echo "-- workflow-unit hydration: panel-consult-cycle-template (source design/workflows/panel-consult-cycle-template.toml) --"
echo "-- led: $LED --"
...
open_phase "panel-consult-cycle-template-implement" "implement phase of panel-consult-cycle-template (from design/workflows/panel-consult-cycle-template.toml)"
PHASE_IMPLEMENT_FRESH=$?

open_phase "panel-consult-cycle-template-compliance-review" "compliance-review phase of panel-consult-cycle-template (from design/workflows/panel-consult-cycle-template.toml)"
PHASE_COMPLIANCE_REVIEW_FRESH=$?

open_phase "panel-consult-cycle-template-compliance-countersign" "compliance-countersign phase of panel-consult-cycle-template (from design/workflows/panel-consult-cycle-template.toml)"
PHASE_COMPLIANCE_COUNTERSIGN_FRESH=$?


if [ "$PHASE_COMPLIANCE_REVIEW_FRESH" -eq 1 ]; then
  out=$("$LED" work depends "panel-consult-cycle-template-compliance-review" "panel-consult-cycle-template-implement" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge panel-consult-cycle-template-compliance-review -> panel-consult-cycle-template-implement" >&2; exit 1; }
else
  echo "panel-consult-cycle-template-compliance-review: skipping depends_on edge to panel-consult-cycle-template-implement (phase already hydrated)"
fi
if [ "$PHASE_COMPLIANCE_COUNTERSIGN_FRESH" -eq 1 ]; then
  out=$("$LED" work depends "panel-consult-cycle-template-compliance-countersign" "panel-consult-cycle-template-compliance-review" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge panel-consult-cycle-template-compliance-countersign -> panel-consult-cycle-template-compliance-review" >&2; exit 1; }
else
  echo "panel-consult-cycle-template-compliance-countersign: skipping depends_on edge to panel-consult-cycle-template-compliance-review (phase already hydrated)"
fi
...
if [ "$NO_OBLIGATE" -eq 1 ]; then
  ...
  if [ "$CONFIRMED" -eq 1 ]; then
    out=$("$LED" obligate "panel-consult-cycle-template-review" "$OBLIGATE_ASSIGNED_BY" "$OBLIGATE_OBLIGED_ACTOR" 2>&1)
    rc=$?
    if [ $rc -eq 0 ]; then
      echo "$out"
...
echo "-- hydration complete for panel-consult-cycle-template --"
