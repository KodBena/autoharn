...
if [ "$PHASE_DECLARE_RESOURCE_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_DECLARE_RESOURCE" "$SLUG_PROVISION" --type blocks-start 2>&1); rc=$?
  echo "$out"
...
if [ "$PHASE_WITNESS_SCHEDULE_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_WITNESS_SCHEDULE" "$SLUG_PROVISION" --type blocks-start 2>&1); rc=$?
  echo "$out"
...
if [ "$PHASE_WITNESS_SCHEDULE_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_WITNESS_SCHEDULE" "$SLUG_DECLARE_RESOURCE" --type blocks-start 2>&1); rc=$?
  echo "$out"
...
if [ "$NO_OBLIGATE" -eq 1 ]; then
  ...
  if [ "$CONFIRMED" -eq 1 ]; then
    out=$(led_run obligate "panel-msched-resource-provisioning-${INSTANCE}-review" "$OBLIGATE_ASSIGNED_BY" "$OBLIGATE_OBLIGED_ACTOR" 2>&1)
    rc=$?
    if [ $rc -eq 0 ]; then
      echo "$out"
