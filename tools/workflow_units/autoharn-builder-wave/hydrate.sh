...
echo "-- workflow-unit hydration: autoharn-builder-wave (source design/workflows/autoharn-builder-wave.toml) --"
echo "-- led: $LED --"
...
open_phase "autoharn-builder-wave-claim" "claim phase of autoharn-builder-wave (from design/workflows/autoharn-builder-wave.toml)"
PHASE_CLAIM_FRESH=$?

open_phase "autoharn-builder-wave-build-and-witness" "build-and-witness phase of autoharn-builder-wave (from design/workflows/autoharn-builder-wave.toml)"
PHASE_BUILD_AND_WITNESS_FRESH=$?

open_phase "autoharn-builder-wave-close-deferred" "close-deferred phase of autoharn-builder-wave (from design/workflows/autoharn-builder-wave.toml)"
PHASE_CLOSE_DEFERRED_FRESH=$?

open_phase "autoharn-builder-wave-orchestrator-merge" "orchestrator-merge phase of autoharn-builder-wave (from design/workflows/autoharn-builder-wave.toml)"
PHASE_ORCHESTRATOR_MERGE_FRESH=$?


if [ "$PHASE_BUILD_AND_WITNESS_FRESH" -eq 1 ]; then
  out=$("$LED" work depends "autoharn-builder-wave-build-and-witness" "autoharn-builder-wave-claim" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge autoharn-builder-wave-build-and-witness -> autoharn-builder-wave-claim" >&2; exit 1; }
else
  echo "autoharn-builder-wave-build-and-witness: skipping depends_on edge to autoharn-builder-wave-claim (phase already hydrated)"
fi
if [ "$PHASE_CLOSE_DEFERRED_FRESH" -eq 1 ]; then
  out=$("$LED" work depends "autoharn-builder-wave-close-deferred" "autoharn-builder-wave-build-and-witness" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge autoharn-builder-wave-close-deferred -> autoharn-builder-wave-build-and-witness" >&2; exit 1; }
else
  echo "autoharn-builder-wave-close-deferred: skipping depends_on edge to autoharn-builder-wave-build-and-witness (phase already hydrated)"
fi
if [ "$PHASE_ORCHESTRATOR_MERGE_FRESH" -eq 1 ]; then
  out=$("$LED" work depends "autoharn-builder-wave-orchestrator-merge" "autoharn-builder-wave-close-deferred" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge autoharn-builder-wave-orchestrator-merge -> autoharn-builder-wave-close-deferred" >&2; exit 1; }
else
  echo "autoharn-builder-wave-orchestrator-merge: skipping depends_on edge to autoharn-builder-wave-close-deferred (phase already hydrated)"
fi
...
if [ "$NO_OBLIGATE" -eq 1 ]; then
  ...
  if [ "$CONFIRMED" -eq 1 ]; then
    out=$("$LED" obligate "autoharn-builder-wave-review" "$OBLIGATE_ASSIGNED_BY" "$OBLIGATE_OBLIGED_ACTOR" 2>&1)
    rc=$?
    if [ $rc -eq 0 ]; then
      echo "$out"
...
echo "-- hydration complete for autoharn-builder-wave --"
