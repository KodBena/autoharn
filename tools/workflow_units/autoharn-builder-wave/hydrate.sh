...
# design/FABLE-WORKFLOW-UNIT-COMPILER-SPEC.md's hydration-script artifact for workflow 'autoharn-builder-wave'.
#
...
# `autoharn-builder-wave-<instance>-<phase>`, `--instance <token>` is MANDATORY, and idempotent-by-refusal
...
usage() {
  ...
  echo "                      slugs are autoharn-builder-wave-<instance>-<phase>. Two different tokens are two" >&2
...
echo "-- workflow-unit hydration: autoharn-builder-wave instance '$INSTANCE' (source design/workflows/autoharn-builder-wave.toml) --"
echo "-- led: $LED --"
...
SLUG_CLAIM="autoharn-builder-wave-${INSTANCE}-claim"
SLUG_BUILD_AND_WITNESS="autoharn-builder-wave-${INSTANCE}-build-and-witness"
SLUG_CLOSE_DEFERRED="autoharn-builder-wave-${INSTANCE}-close-deferred"
SLUG_ORCHESTRATOR_MERGE="autoharn-builder-wave-${INSTANCE}-orchestrator-merge"

open_phase "$SLUG_CLAIM" "claim phase of autoharn-builder-wave instance '${INSTANCE}' (from design/workflows/autoharn-builder-wave.toml)"
PHASE_CLAIM_FRESH=$?

open_phase "$SLUG_BUILD_AND_WITNESS" "build-and-witness phase of autoharn-builder-wave instance '${INSTANCE}' (from design/workflows/autoharn-builder-wave.toml)"
PHASE_BUILD_AND_WITNESS_FRESH=$?

open_phase "$SLUG_CLOSE_DEFERRED" "close-deferred phase of autoharn-builder-wave instance '${INSTANCE}' (from design/workflows/autoharn-builder-wave.toml)"
PHASE_CLOSE_DEFERRED_FRESH=$?

open_phase "$SLUG_ORCHESTRATOR_MERGE" "orchestrator-merge phase of autoharn-builder-wave instance '${INSTANCE}' (from design/workflows/autoharn-builder-wave.toml)"
PHASE_ORCHESTRATOR_MERGE_FRESH=$?


if [ "$PHASE_BUILD_AND_WITNESS_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_BUILD_AND_WITNESS" "$SLUG_CLAIM" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge $SLUG_BUILD_AND_WITNESS -> $SLUG_CLAIM" >&2; exit 1; }
else
  echo "$SLUG_BUILD_AND_WITNESS: skipping depends_on edge to $SLUG_CLAIM (phase already hydrated)"
fi
if [ "$PHASE_CLOSE_DEFERRED_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_CLOSE_DEFERRED" "$SLUG_BUILD_AND_WITNESS" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge $SLUG_CLOSE_DEFERRED -> $SLUG_BUILD_AND_WITNESS" >&2; exit 1; }
else
  echo "$SLUG_CLOSE_DEFERRED: skipping depends_on edge to $SLUG_BUILD_AND_WITNESS (phase already hydrated)"
fi
if [ "$PHASE_ORCHESTRATOR_MERGE_FRESH" -eq 1 ]; then
  out=$(led_run work depends "$SLUG_ORCHESTRATOR_MERGE" "$SLUG_CLOSE_DEFERRED" --type blocks-start 2>&1); rc=$?
  echo "$out"
  [ $rc -eq 0 ] || { echo "$0: REFUSED writing depends edge $SLUG_ORCHESTRATOR_MERGE -> $SLUG_CLOSE_DEFERRED" >&2; exit 1; }
else
  echo "$SLUG_ORCHESTRATOR_MERGE: skipping depends_on edge to $SLUG_CLOSE_DEFERRED (phase already hydrated)"
fi
...
if [ "$NO_OBLIGATE" -eq 1 ]; then
  ...
  if [ "$CONFIRMED" -eq 1 ]; then
    out=$(led_run obligate "autoharn-builder-wave-${INSTANCE}-review" "$OBLIGATE_ASSIGNED_BY" "$OBLIGATE_OBLIGED_ACTOR" 2>&1)
    rc=$?
    if [ $rc -eq 0 ]; then
      echo "$out"
...
echo "-- hydration complete for autoharn-builder-wave instance '$INSTANCE' --"
