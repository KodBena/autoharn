...
# `faq-abc-fixpoint-loop-<instance>-<phase>`, `--instance <token>` is MANDATORY, and idempotent-by-refusal
...
usage() {
  ...
  echo "                      slugs are faq-abc-fixpoint-loop-<instance>-<phase>. Two different tokens are two" >&2
...
echo "-- workflow-unit hydration: faq-abc-fixpoint-loop instance '$INSTANCE' (source design/workflows/faq-abc-fixpoint-loop.toml) --"
echo "-- led: $LED --"
...
SLUG_AUTHOR_DRAFT="faq-abc-fixpoint-loop-${INSTANCE}-author-draft"
SLUG_FRESH_CONTEXT_REVIEW="faq-abc-fixpoint-loop-${INSTANCE}-fresh-context-review"
SLUG_ADJUDICATE="faq-abc-fixpoint-loop-${INSTANCE}-adjudicate"

open_phase "$SLUG_AUTHOR_DRAFT" "author-draft phase of faq-abc-fixpoint-loop instance '${INSTANCE}' (from design/workflows/faq-abc-fixpoint-loop.toml)"
PHASE_AUTHOR_DRAFT_FRESH=$?

open_phase "$SLUG_FRESH_CONTEXT_REVIEW" "fresh-context-review phase of faq-abc-fixpoint-loop instance '${INSTANCE}' (from design/workflows/faq-abc-fixpoint-loop.toml)"
PHASE_FRESH_CONTEXT_REVIEW_FRESH=$?

open_phase "$SLUG_ADJUDICATE" "adjudicate phase of faq-abc-fixpoint-loop instance '${INSTANCE}' (from design/workflows/faq-abc-fixpoint-loop.toml)"
...
if [ "$PHASE_ADJUDICATE_FRESH" -eq 1 ]; then
  ...
  echo "$SLUG_ADJUDICATE: skipping depends_on edge to $SLUG_FRESH_CONTEXT_REVIEW (phase already hydrated)"
fi
...
echo "-- hydration complete for faq-abc-fixpoint-loop instance '$INSTANCE' --"
