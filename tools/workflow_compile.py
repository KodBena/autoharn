...
Usage:
    python3 tools/workflow_compile.py <path-to.toml> [--out-dir DIR] [--repo-root DIR]

--repo-root DIR (or the AUTOHARN_REPO_ROOT env var; the flag wins if both are given) overrides
this compiler's own idea of the repo root used to bake ROLE_CHARTER_PY/ROLE_BRIEF_PY (see
resolve_repo_root() below) -- needed when recompiling from a worktree checkout, whose own
`Path(__file__).resolve().parents[1]` is the worktree's transient path, never the real checkout
an operator wants baked into a committed drive.py. A nonexistent DIR is refused (exit 2), never
silently accepted.
...
def resolve_repo_root(override: str | None) -> Path:
    ...
    runs from the canonical checkout, but WRONG when invoked from a worktree checkout (e.g.
    .claude/worktrees/<agent>/tools/workflow_compile.py) -- it would bake that worktree's own
    transient path into ROLE_CHARTER_PY/ROLE_BRIEF_PY below, which is never the path a real
    operator wants committed. `override` (an explicit --repo-root flag value, or the
    AUTOHARN_REPO_ROOT env var, checked by the caller in that priority order) lets an operator
    regenerating from a worktree name the REAL checkout's root explicitly, so the baked absolute
    paths are correct without hand-editing after the fact. A nonexistent override is refused
    loudly here (exit 2), never silently accepted and baked as a dangling path."""
    if override is None:
        return Path(__file__).resolve().parents[1]
    root = Path(override).expanduser().resolve()
    if not root.is_dir():
        print(f"workflow_compile: REFUSED -- --repo-root/AUTOHARN_REPO_ROOT "
              f"'{override}' does not exist or is not a directory.", file=sys.stderr)
        raise SystemExit(2)
    return root


# SCRIPT_ROOT is NEVER overridden -- it anchors the default --out-dir (where THIS invocation of
# the compiler writes its output), which must stay tied to the checkout actually running the
# script (e.g. this worktree's own tools/workflow_units/) even when --repo-root/AUTOHARN_REPO_ROOT
# points somewhere else for the baked-path purpose below. Conflating the two would make an
# override silently redirect output into a DIFFERENT checkout entirely -- caught in testing this
# fix, when a --repo-root pointed at the main checkout wrote regenerated units there instead of
# into this worktree.
...
# two tools. Baked at COMPILE time (this constant, known-good regardless of --out-dir) rather
# than resolved at RUN time relative to the generated script's own location, because --out-dir
# is a caller-controlled compile-time option and a generated drive.py may be invoked from an
# arbitrary CWD (a scaffolded world's own root, per its --led default) -- the SAME reasoning
# `led`'s own AUTOHARN_ROOT convention documents, applied here as a literal baked constant
# instead of an env-var-overridable RUN-time resolution (this artifact is regenerated by
# recompiling, never hand-edited, so a stale baked path is a recompile away from correct, not a
# maintenance burden). The COMPILER's own idea of REPO_ROOT, however, IS overridable -- via
# --repo-root or the AUTOHARN_REPO_ROOT env var (--repo-root wins if both given) -- precisely so
# that recompiling from a worktree checkout can bake the real checkout's path rather than the
# worktree's transient one; see resolve_repo_root() above and main()'s --repo-root handling
# below, which re-resolves these same three constants before compiling.
REPO_ROOT = resolve_repo_root(os.environ.get("AUTOHARN_REPO_ROOT"))
...
if [ "$NO_OBLIGATE" -eq 1 ]; then
  ...
  if [ "$CONFIRMED" -eq 1 ]; then
    out=$(led_run obligate "{stem}-${{INSTANCE}}-review" "$OBLIGATE_ASSIGNED_BY" "$OBLIGATE_OBLIGED_ACTOR" 2>&1)
    rc=$?
    if [ $rc -eq 0 ]; then
      echo "$out"
...
def render_hydrate(stem: str, toml_rel: str, doc: dict) -> str:
    ...
    for phase, dep in depends_edges(doc):
        ...
        depends_lines.append(
            ...
            f'  out=$(led_run work depends "$SLUG_{ident}" "$SLUG_{dep_ident}" --type blocks-start 2>&1); rc=$?\n'
            f'  echo "$out"\n'
...
def compile_toml(toml_path: Path, out_dir: Path) -> tuple[Path, Path]:
    ...
    stem = toml_path.stem
    try:
        toml_rel = str(toml_path.resolve().relative_to(SCRIPT_ROOT))
    except ValueError:
...
def main(argv: list[str]) -> int:
    global REPO_ROOT, WORKFLOW_CHECK, ROLE_CHARTER_PY, ROLE_BRIEF_PY
    if not argv:
        print("usage: python3 tools/workflow_compile.py <path-to.toml> "
              "[--out-dir DIR] [--repo-root DIR]", file=sys.stderr)
        return 2
    toml_arg = argv[0]
    ...
    while i < len(argv):
        if argv[i] == "--out-dir":
            ...
            repo_root_arg = argv[i + 1]
            i += 2
        else:
            print(f"unrecognized argument: {argv[i]}", file=sys.stderr)
            return 2

    # --repo-root (flag wins over AUTOHARN_REPO_ROOT if both given) re-resolves the module's
    # baked-path constants BEFORE compiling, so a regeneration invoked from a worktree checkout
    # can bake the real checkout's absolute paths into ROLE_CHARTER_PY/ROLE_BRIEF_PY instead of
    # the worktree's own transient one -- see resolve_repo_root()'s docstring above.
    ...
    if out_dir is None:
        out_dir = SCRIPT_ROOT / "tools" / "workflow_units"
