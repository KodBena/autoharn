...
**1. Differential solving (kills solver-error and some faithfulness error; the [INDEP](KEY.md#indep) backbone).** Compile the *same source-of-truth obligation* into two engines with no common code lineage and require bit-identical verdicts on every fixture. This is leverageable *today* on the installed stack because nearly every formalism in this survey has a dissimilar second host:

| Obligation family | Primary | Dissimilar differential oracle |
|---|---|---|
| [PROV](KEY.md#prov)/TRACE/AUTH (Datalog/ASP) | clingo 5.8.0 | Soufflé (UPL) / SWI tabled Prolog / Postgres recursive |
| [INV](KEY.md#inv)/STRUCT/COHERE (SMT) | Z3 4.16.0 | cvc5 (different codebase, Alethe proofs) |
| [INV](KEY.md#inv)/PROG (temporal) | NuSMV / TLC | Spot `ltlcross`; Apalache (Z3-backed) vs TLC |
| [CLASS](KEY.md#class)/CONSIST (finite) | OR-Tools CP-SAT | MiniZinc → Chuffed/Gecode; clingo |
...
Take the [AUTH](KEY.md#auth) encoding from the Prolog and ASP sections (spillway deploy gate). QUAL-2 + QUAL-4, fully mechanical, no LLM in the verdict path:
