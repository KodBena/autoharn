...
class CapabilityManifest(BaseModel):
    ...
    s70_scope_filter: bool = Field(description="s70's principal_scopes view present (kernel/lineage/s70-scope-binding.sql Element 5): schema.principal_scopes -- AND this service's own boundary-side scope filter is wired in (serving/boundary_scope_filter.py, design/FABLE-ACCESS-CONTROL-AND-INFORMATION-FLOW-SPEC.md sec1b/1c/2/4, work item ac-boundary-scope-filter). Object existence only, same discipline as every field above: a minted-principal caller's GET reads are filtered by their current in-force principal_scopes row iff this is true; false means every read serves byte-identically to a pre-s70 world (the fail-safe/regression-bar default), whether because the kernel lacks s70 or (never observed in this build) the filter itself were disabled.")
...
class MetaResponse(BaseModel):
    ...
    read_identity_journal: bool = Field(default=True, description="design/FABLE-ACCESS-CONTROL-AND-INFORMATION-FLOW-SPEC.md §1a: every GET this boundary serves resolves an identity through the same three-case conduit the write path already uses (vendor stamp / minted principal / anonymous) and journals who-read-what -- route, resolved identity, a bare row count, never row content -- to <world_dir>/.claude/logs/boundary_reads.jsonl. Reads are never refused for anonymity; this field is a positive capability confirmation, not a gate. Fixed true for every deployment this service build serves (no world predates it) -- named as a field rather than left to be inferred from absence. ADDITIVE field, no protocol_version bump.")
